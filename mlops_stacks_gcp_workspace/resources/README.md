...
The databricks CLI bundles top level is defined by file `mlops_stacks_gcp_workspace/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`mlops_stacks_gcp_workspace/resources/model-workflow-resource.yml`)
 - batch inference workflow (`mlops_stacks_gcp_workspace/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`mlops_stacks_gcp_workspace/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`mlops_stacks_gcp_workspace/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`mlops_stacks_gcp_workspace/resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-mlops_stacks_gcp_workspace-model     | /dev-mlops_stacks_gcp_workspace-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-mlops_stacks_gcp_workspace-model | /staging-mlops_stacks_gcp_workspace-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-mlops_stacks_gcp_workspace-model    | /prod-mlops_stacks_gcp_workspace-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-mlops_stacks_gcp_workspace-model    | /test-mlops_stacks_gcp_workspace-experiment    |
...
2. Complete the `TODO` in `mlops_stacks_gcp_workspace/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `mlops_stacks_gcp_workspace/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `mlops_stacks_gcp_workspace/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-mlops_stacks_gcp_workspace-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-mlops_stacks_gcp_workspace-batch-inference-job`
that runs the notebook under `mlops_stacks_gcp_workspace/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-mlops_stacks_gcp_workspace-batch-inference-job`.
...
`test-mlops_stacks_gcp_workspace-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-mlops_stacks_gcp_workspace-batch-inference-job`.
